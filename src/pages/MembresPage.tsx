...
const DEPARTEMENTS = ['Louange', 'Intercession', 'Évangélisation', 'Technique', 'Accueil', 'Administration', 'Jeunesse', 'Enfants']
...
export default function MembresPage() {
  ...
  const fetchMembres = async () => {
    setLoading(true)
    const { data } = await supabase.from('membres').select('*, personnes(*)').eq('actif', true).order('created_at', { ascending: false })
    setMembres(data || [])
    setLoading(false)
  }

  const fetchPersonnes = async () => {
    // Personnes qui ne sont pas encore membres actifs
  ...
  // --- Formulaire ---
  const MembreForm = ({ isEdit }: { isEdit?: boolean }) => (
    <div className="space-y-4">
      <div>
        ...
        {isEdit ? (
          ...
          <select className="input" value={form.personne_id} onChange={e => setForm(f => ({ ...f, personne_id: e.target.value }))}>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input className="input" value={form.numero_membre} onChange={e => setForm(f => ({ ...f, numero_membre: e.target.value }))} placeholder="Ex : MBR-2025-001" />
        </div>
        <div>
          <label className="label">Date d'adhésion</label>
          <input type="date" className="input" value={form.date_adhesion} onChange={e => setForm(f => ({ ...f, date_adhesion: e.target.value }))} />
        </div>
        <div>
        ...
        <div>
          ...
          <input type="date" className="input" value={form.date_liberation} onChange={e => setForm(f => ({ ...f, date_liberation: e.target.value }))} />
        </div>
        <div>
          <label className="label">Motif de libération</label>
          <input className="input" value={form.motif_liberation} onChange={e => setForm(f => ({ ...f, motif_liberation: e.target.value }))} />
        </div>
      </div>
    </div>
  )
    ...
    <div className="space-y-6">
      ...
      <div className="card">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="relative">
            ...
            <input className="input pl-9" placeholder="Nom, prénom, n° membre..." value={search} onChange={e => setSearch(e.target.value)} />
          </div>
      ...
      <div className="card overflow-hidden p-0">
        {loading ? (
          ...
          <EmptyState message="Aucun membre trouvé" />
        ) : (
      ...
      {/* Modal Modifier */}
      <Modal isOpen={editModal} onClose={() => setEditModal(false)} title={`Modifier — ${editItem?.personnes?.prenom} ${editItem?.personnes?.nom}`} size="lg">
        <MembreForm isEdit />
        <div className="flex justify-end gap-3 mt-6 pt-4 border-t">
      ...
      {/* Modal Visualiser */}
      <Modal isOpen={viewModal} onClose={() => setViewModal(false)} title={`Fiche membre — ${viewItem?.personnes?.prenom} ${viewItem?.personnes?.nom}`} size="md">
        {viewItem && (
